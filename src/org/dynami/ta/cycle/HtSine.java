 ...
 * GENERATED CODE
 */
public class HtSine extends TaLibIndicator implements ITechnicalIndicator {
	private int lastLength = 0;
	private int PAD = 4;
	private boolean ready = false;
	// output series
	private Series outSine = new Series();
	private Series outLeadSine = new Series();
	...
	public String getDescription(){
		return "Ht Sine - Cycle Indicators";
	}
	...
	public void compute( final Series inReal) {
		...
		final double[] _outSine = new double[_tmpinReal.length];
		final double[] _outLeadSine = new double[_tmpinReal.length];
		
		ready = DUtils.max(_tmpinReal.length) >= DUtils.max();
		// calculate output
		core.htSine(0, _tmpinReal.length-1, _tmpinReal, outBegIdx, outNBElement, _outSine, _outLeadSine);
 		// shift data to end of array and set output fields
		DUtils.shift(_outSine, outBegIdx.value);
		DUtils.shift(_outLeadSine, outBegIdx.value);
	...
	public Series getLeadSine(){
		return outLeadSine;
	}

	@Override
	public List<Supplier<Series>> series() {
		return Arrays.asList(this::getSine, this::getLeadSine);
	}
	@Override
	...
	public String[] seriesNames() {
		return new String[]{"Sine", "LeadSine"};
	}
}

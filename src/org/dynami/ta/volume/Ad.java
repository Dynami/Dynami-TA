 ...
 * GENERATED CODE
 */
public class Ad extends TaLibIndicator implements ITechnicalIndicator {
	...
	public void compute( final Series high, final Series low, final Series close, final Series volume) {
		...
		ready = DUtils.max(_tmphigh.length, _tmplow.length, _tmpclose.length, _tmpvolume.length) >= DUtils.max();
		// calculate output
		core.ad(0, _tmphigh.length-1, _tmphigh, _tmplow, _tmpclose, _tmpvolume, outBegIdx, outNBElement, _outReal);

 ...
 * GENERATED CODE
 */
public class UltOsc extends TaLibIndicator {
	private int firstPeriod = 7;
	private int secondPeriod = 14;
	private int thirdPeriod = 28;
	// output series
	private Series outReal = new Series();
	
	/**
	 * Default constructor with standard input parameters</br>
	 * default value for firstPeriod = 7</br>
	 * default value for secondPeriod = 14</br>
	 * default value for thirdPeriod = 28</br>
	 */
	public UltOsc(){
		computePad( firstPeriod,  secondPeriod,  thirdPeriod, PAD);
	}

	/**
	 * Default constructor with custom input parameters:
	 * @param firstPeriod default value 7
	 * @param secondPeriod default value 14
	 * @param thirdPeriod default value 28
	 */
	public UltOsc(int firstPeriod, int secondPeriod, int thirdPeriod){
	...
	public String getDescription(){
		return "Ult Osc - Momentum Indicators";
	}
	 ...
	 * and input Series.
	 */
	public UltOsc compute( final Series high, final Series low, final Series close) {
		...
		ready = DUtils.max(_tmphigh.length, _tmplow.length, _tmpclose.length) >= DUtils.max(firstPeriod, secondPeriod, thirdPeriod);
		// calculate output
		core.ultOsc(0, _tmphigh.length-1, _tmphigh, _tmplow, _tmpclose, this.firstPeriod, this.secondPeriod, this.thirdPeriod, outBegIdx, outNBElement, _outReal);

 ...
 * GENERATED CODE
 */
public class StochRsi extends TaLibIndicator {
	 ...
	 */
	public StochRsi(){
		computePad( timePeriod,  fast_KPeriod,  fast_DPeriod, PAD);
	}
	 ...
	 */
	public StochRsi(int timePeriod, int fast_KPeriod, int fast_DPeriod, MAType fast_DMA){
	...
	public String getDescription(){
		return "Stoch Rsi - Momentum Indicators";
	}
	 ...
	 * and input Series.
	 */
	public StochRsi compute( final Series inReal) {
		...
		final double[] _outFastK = new double[_tmpinReal.length];
		final double[] _outFastD = new double[_tmpinReal.length];
		
		ready = DUtils.max(_tmpinReal.length) >= DUtils.max(timePeriod, fast_KPeriod, fast_DPeriod);
		// calculate output
		core.stochRsi(0, _tmpinReal.length-1, _tmpinReal, this.timePeriod, this.fast_KPeriod, this.fast_DPeriod, this.fast_DMA, outBegIdx, outNBElement, _outFastK, _outFastD);
